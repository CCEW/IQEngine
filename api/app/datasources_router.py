...
async def integration_query(current_user: Optional[dict] = Depends(get_current_user), account: Optional[List[str]] = Query([]), container: Optional[List[str]] = Query([]), min_frequency: Optional[float] = Query(None), max_frequency: Optional[float] = Query(None), min_datetime: Optional[datetime] = Query(None), max_datetime: Optional[datetime] = Query(None), author: Optional[str] = Query(None), location: Optional[str] = Query(None), signal_type: Optional[str] = Query(None), hw: Optional[str] = Query(None), operator: Optional[str] = Query(None), recorder: Optional[str] = Query(None), text: Optional[str] = Query(None)):
    results = await query_metadata(account=account, container=container, min_frequency=min_frequency, max_frequency=max_frequency, min_datetime=min_datetime, max_datetime=max_datetime, author=author, location=location, signal_type=signal_type, hw=hw, operator=operator, recorder=recorder, text=text, catalog_status="active")
